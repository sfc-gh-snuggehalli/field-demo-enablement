...
- An evals harness (scoring accuracy, run-to-run consistency, coached win-rate lift)
...
- **ACCOUNTADMIN** for the OAuth security integration and `ALTER USER ... DEFAULT_ROLE` in Part A.
- Cortex enabled (the `SNOWFLAKE.CORTEX_USER` database role).
- For live traces: `MONITOR` on the supervisor agent + `SNOWFLAKE.CORTEX_USER`.
...
- Shared logging tables: `REQUEST_LOG`, `ROUTING_LOG`, `COST_COMPARISON`, `EVAL_RUNS`
...
4. **gtm-04-evals** — build a labeled set and compute scoring accuracy, consistency, and coached win-rate lift; persist runs (Checkpoint C).
